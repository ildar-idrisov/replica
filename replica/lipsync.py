...
def face_detect(images, nosmooth, pads, device):
...
def datagen(frames, mels, nosmooth, pads, device):
	img_batch, mel_batch, frame_batch, coords_batch = [], [], [], []

	face_det_results = face_detect(frames, nosmooth, pads, device) # BGR2RGB for CNN face detection
...
def inference(checkpoint_path, video_file, audio_file, resize_factor, nosmooth, output_file, device, pads = (0, 10, 0, 0)):
	if not os.path.isfile(video_file):
		raise ValueError('--video_file argument must be a valid path to video/image file')
	else:
